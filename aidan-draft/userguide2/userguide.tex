...
Beluga supports proof interactive development, offering features to assist the user as she programs.  Interactive mode includes a command line interface but we strongly recommend leveraging Emacs integration.

\subsection{Keybindings}

\begin{tabular}{ l r }
\shellcmd{C-c C-l filename.bel} & Load \\
\shellcmd{C-c C-x command} & Execute Command \\
\shellcmd{C-c C-c} & Compile \\
\shellcmd{C-c C-s int id} & Split, where \texttt{id} specifies the variable \\
\shellcmd{C-c C-i int} & Variable introduction \\
\shellcmd{C-c C-t} & Type-check at cursor location \\
\end{tabular}

The integers are base-0 indexes denoting the relative position of the hole in question, ie: 0 refers to the first hole, 1 refers to the second hole, etc... 
\subsection{Type Information}
The user can type-check code which is still incomplete. 
...
Interactive mode supports variable splitting to fill computation-level holes with pattern matching. Use the split command to specify the particular hole and variable, respectively.
\shellcmd{split int var\_id}
 Beluga will replace the hole with case analysis, generating cases to cover all possible instances of the particular variable. 
